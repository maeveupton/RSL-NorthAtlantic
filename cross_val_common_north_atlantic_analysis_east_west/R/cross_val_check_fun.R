...
cross_val_check_fun <- function(data, data_grid,
  ...
  for (i in 1:n_fold) {
    ...
    west_data <- input_train$data %>% # input_train_data%>%
      filter(Longitude < -40) %>%
      filter(Latitude < 53) %>%
      mutate(section = "West")
    # east data: europe
    east_data <- anti_join(input_train$data, west_data)
    east_data <- east_data %>% mutate(section = "East")
    data <- rbind(east_data, west_data)
    # West data grid
    west_data_grid <- input_train$data_grid %>%
      filter(Longitude < -40) %>%
      filter(Latitude < 53) %>%
      mutate(section = "West")
    # East data grid
    east_data_grid <- anti_join(input_train$data_grid, west_data_grid)
