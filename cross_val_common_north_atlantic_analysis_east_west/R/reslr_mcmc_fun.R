...
reslr_mcmc_fun <- function(data, data_grid,
  ...
  data <- data %>% mutate(SiteName = as.factor(SiteName))
  data_grid <- data_grid %>% mutate(SiteName = as.factor(SiteName))
  ...
  jags_data <- list(
    ...
    n_sites = length(unique(data$SiteName)),
    n_obs = nrow(data),
    B_r = spline_basis_fun_list$B_t,
    n_knots_r = ncol(spline_basis_fun_list$B_t),
    B_c = spline_basis_fun_list$B_c,
    n_knots_c = ncol(spline_basis_fun_list$B_c),
    linear_rate = data %>%
      dplyr::group_by(SiteName) %>%
  ...
  jags_data <- list(
    ...
    NI_var_term_pred = data_grid$NI_var_grid_term,
    b_r_value = model_run$BUGSoutput$median$b_r,
    b_r_sd_value = model_run$BUGSoutput$sd$b_r,
    h_value = model_run$BUGSoutput$median$intercept,
    h_sd_value = model_run$BUGSoutput$sd$intercept,
    y = data$RSL,
    y_err = data$RSL_err,
    y_err_pred = data_grid$RSL_err,
    t = data$Age,
    t_pred = data_grid$Age,
    site = as.factor(data$SiteName),
    ...
    n_knots_c = ncol(spline_basis_fun_list$B_c),
    B_r = spline_basis_fun_list$B_t,
    B_r_deriv = spline_basis_fun_list$B_t_deriv,
    B_r_pred = spline_basis_fun_list$B_t_pred,
    B_r_pred_deriv = spline_basis_fun_list$B_t_pred_deriv,
    n_knots_r = ncol(spline_basis_fun_list$B_t),
