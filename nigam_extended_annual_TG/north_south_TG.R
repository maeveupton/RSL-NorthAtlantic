# New approach for RSL analysis of North and South of Cape Hatteras using tide gauge data

# Remove common and examine data balanced afterwards

# f(x,t) = c(t) + r_south(t)+ r_north(t) + gt + l(x,t) + Error
...
setwd("/Users/maeve.upton/Dropbox/PhD_Work_2019_2023/RSL_North_Atlantic_chp3/Codes_1Nov/nigam_extended_annual_TG")
#----------Load packages--------------------
library(devtools)
...
# R functions-------
source("R/plot_data.R") # Plotting separate south & north data & map plot
source("R/internal_functions.R") # New basis functions for south & north data
source("R/reslr_mcmc_fun.R") # New basis functions for south & north data
source("R/reslr_load_fun.R")

# Read in PSMSL annual data wide format:
tg_data <- read_csv("DATA.csv")
tg_data_clean <- tg_data %>% 
  pivot_longer(!Year,names_to = "Site",values_to = "RSL") %>% 
  mutate(RSL = RSL - 7,
         RSL_err = 0.001) %>% #PSMSL weird 7000
  rename(Age = Year) %>% 
  drop_na()
# Long lat info for tg:
info_tg <- read_csv("META.csv") %>% 
  t() %>% 
  as.data.frame() %>% 
  ...
  mutate(Age_err = 0,
         Region = "",
         data_type_id = "TideGaugeData")
# Join info with Tide gauges
tg_df <- left_join(tg_data_clean,info_tg, by = c("Site" = "Site") )
...
north_at_reslr <- reslr_load_fun(
  ...
  all_TG_1deg = FALSE,
  prediction_grid_res = 1#Annual data so keep this at 1 30#1
)
...
data_plot_north <- plot_data(north_data,
                            ...
                            plot_proxy_records = FALSE)
ggsave(data_plot_north, filename = "fig/full_dataset/data_plot_north.pdf", height = 6, width = 10)
data_plot_south <- plot_data(south_data,
                            plot_tide_gauges = TRUE,
                            plot_proxy_records = FALSE)
ggsave(data_plot_south, filename = "fig/full_dataset/data_plot_south.pdf", height = 6, width = 10)
...
#     spline_nseg_c = 15#20
#   )
# saveRDS(global_output,file = "reslr_outputs/common_only_reslr_output_pred_1_c15.rds")

# Changing smoothness settings
global_output <- readRDS("reslr_outputs/common_only_reslr_output_pred_1_c15.rds")
# Full Model Output use pred grid = 1
#global_output <- readRDS("reslr_outputs/common_only_reslr_output_pred_1.rds")# t = 8,c = 20,st = 6

#global_output <- readRDS("reslr_outputs/common_only_reslr_output_pred_30.rds")# Full Model Output
#global_output <- readRDS("reslr_outputs/common_only_reslr_output_pred_10.rds")# Full Model Output
...
tot_plot <-
  ...
  geom_line(data = tot_post_df_full, aes(x = Age, y = pred,colour = "mean"))+
  geom_ribbon(data = tot_post_df_full, aes(x = Age,ymin = lwr,ymax=upr,fill = "CI"),alpha = 0.2)+
  geom_ribbon(data = tot_post_df_full,aes(x = Age,ymin = lwr_50,ymax=upr_50,fill = "CI"),alpha = 0.3)+
  ggplot2::theme_bw() +
  ggplot2::theme(
...
tot_rate_plot <- ggplot(data = tot_rate_post_df_full, aes(x = Age, y = pred))+
...
l_plot <- ggplot()+
  geom_line(data = l_post_df_full, aes(x = Age, y = pred,colour = "mean"))+
  geom_ribbon(data = l_post_df_full,aes(x = Age,ymin = lwr,ymax=upr,fill = "CI"),alpha = 0.2)+
  geom_ribbon(data = l_post_df_full,aes(x = Age,ymin = lwr_50,ymax=upr_50,fill = "CI"),alpha = 0.3)+
  ggplot2::theme_bw() +
  ggplot2::theme(
    strip.text.x = ggplot2::element_text(size = 7),
    strip.background = ggplot2::element_rect(fill = c("white"))
...
l_rate_plot <- ggplot()+
  geom_line(data = l_post_rate_df_full, aes(x = Age, y = pred,colour = "mean"))+
  geom_ribbon(data = l_post_rate_df_full,aes(x = Age,ymin = lwr,ymax=upr,fill = "CI"),alpha = 0.2)+
  geom_ribbon(data = l_post_rate_df_full,aes(x = Age,ymin = lwr_50,ymax=upr_50,fill = "CI"),alpha = 0.3)+
  ggplot2::theme_bw() +
  ggplot2::geom_hline(yintercept = 0) +
  ggplot2::theme(
    strip.text.x = ggplot2::element_text(size = 7),
...
l_gia_plot <- ggplot()+
  geom_line(data = l_gia_post_df_full, aes(x = Age, y = pred,colour = "mean"))+
  geom_ribbon(data = l_gia_post_df_full,aes(x = Age,ymin = lwr,ymax=upr,fill = "CI"),alpha = 0.2)+
  geom_ribbon(data = l_gia_post_df_full,aes(x = Age,ymin = lwr_50,ymax=upr_50,fill = "CI"),alpha = 0.3)+
  ggplot2::theme_bw() +
  ggplot2::theme(
    strip.text.x = ggplot2::element_text(size = 7),
    strip.background = ggplot2::element_rect(fill = c("white"))
...
reg_plot
ggsave(reg_plot,filename = "fig/full_dataset/reg_south_north_southonly.pdf", width = 10, height = 6)
...
reg_rate_plot
ggsave(reg_rate_plot,filename = "fig/full_dataset/rate_reg_south_north.pdf", width = 10, height = 6)
...
diff_reg_plot
ggsave(diff_reg_plot,filename = "fig/full_dataset/diff_reg_south_north.pdf", width = 10, height = 6)

#write_csv(diff_r_post_df,"Upton_reg_diff_south_north.csv")
...
diff_reg_rate_plot
ggsave(diff_reg_rate_plot,filename = "fig/full_dataset/diff_reg_rate_south_north.pdf", width = 10, height = 6)
