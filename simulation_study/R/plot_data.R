...
plot_map <- function(data,save_name){
  # Plot map of data:
  ...
  SL_df_unique <- data %>%  
    ...
    mutate(SiteName = as.character(SiteName),
           data_type_id = factor(data_type_id,levels = c("ProxyRecord","TideGaugeData")))
  map_locations <- ggplot(data = world) +
    ...
    coord_sf(xlim = c(-98, 20), ylim = c(20, 70), expand = FALSE)+
    # East West
    geom_vline(xintercept = -40,linetype = "dashed")+
    geom_label(aes(x = -35, y = 58,label = "East"), fill = "white",fontface="bold",size = 4)+
    geom_label(aes(x = -50, y = 58, label = "West"),fill = "white",fontface="bold",size = 4)+
    # Cape Hatteras Latitude: 35.2546 Longitude: -75.5200.
    geom_segment(aes(x = -100 , y = 35.3, xend = -40, yend = 35.3),linetype = "dashed") +
    geom_label(aes(x = -60, y = 35.3,
                     label = "Cape Hatteras, USA"),
                  fill = "white",fontface="bold",size = 3) +
    geom_label(aes(x = -60, y = 40,label = "North"), fill = "white",fontface="bold",size = 4)+
    geom_label(aes(x = -60, y = 30, label = "South"),fill = "white",fontface="bold",size = 4)+
    labs(x="Longitude",y="Latitude")+
    annotation_scale(location = "br", width_hint = 0.5)+
    geom_label(aes(x = -40,y=50),label = "North Atlantic Ocean",fill = "lightblue1",
  ...
  map_locations
  ggsave(map_locations,filename =save_name, width = 10, height = 6)
